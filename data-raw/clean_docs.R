...
excel_sheets('data-raw/CVPIA_FloodplainAreas_docs.xlsx')

doc_templates <- read_excel('data-raw/CVPIA_FloodplainAreas_docs.xlsx', sheet = 'DocTemplate')
...
doc_templates$details_fallrun[1]
# modeling details------------------------------------
print_model_details <- function(ws, species) {
  ...
  if (species == 'sr' & is.na(watershed_doc_vars$SR_length_modeled_mi)) {
    stop(sprintf('There are no spring run in %s.', ws))
  }
